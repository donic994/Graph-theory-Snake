...
The position and orientation of the snake should be determined randomly, as should the position of the three squares. 
The size of the snake remains the same when collecting new squares.
It is necessary to find the shortest path by which the snake would pick up all three squares 
...
def createSnake():
    ...
    if(r1 <= 8 and r2 < 10):
        snake.append([r1, r2])
        snake.append([r1, r2+1])
        snake.append([r1, r2+2])
        snake.append([r1, r2+3])
        snake.append([r1, r2+4])
        snake.append([r1, r2+5])
        snake.append([r1, r2+6])
    elif(r2 <= 8):
        snake.append([r1, r2])
        snake.append([r1-1, r2])
        snake.append([r1-2, r2])
        snake.append([r1-3, r2])
        snake.append([r1-4, r2])
        snake.append([r1-5, r2])
        snake.append([r1-6, r2])
    else:
        snake.append([r1, r2])
        snake.append([r1, r2-1])
        snake.append([r1, r2-2])
...
def showSnake():
    for var in snake:
        x1 = var[0]
        y1 = var[1]
        rectangle = w.create_rectangle(
            x1*20-20, y1*20-20, x1*20, y1*20, fill='#22bb45')
        snakeBodyParts.append(rectangle)
...
def move():
    while (True):
        ...
        if(len(food) == 3):
            ...
            print("Food3 = " + str(food[2]))
            print("Head = " + str(snake[0]))
            if (str(snake[0]) == str(food[2])):
                food.remove(food[2])
                w.delete(foodObject[2])
...
def closestFood(head):
    ...
    if(len(food) == 1):
        ...
        paths = []
        paths.append
        paths.append(graph.dijkstra(str(head), str(food[0])))
        paths.append(graph.dijkstra(str(head), str(food[1])))
        paths.append(graph.dijkstra(str(head), str(food[2])))
        paths.sort(key=len)
        path = paths[0]
